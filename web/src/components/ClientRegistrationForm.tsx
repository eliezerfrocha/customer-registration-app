...
export function ClientRegistrationForm({ onColorPreview }: ClientRegistrationFormProps) {
  ...
  function focusFirstInvalidField(errors: ClientFormErrors) {
    const order: (keyof ClientFormValues)[] = ["fullName", "cpf", "email", "colorId", "notes"];
    const firstField = order.find((field) => errors[field]);
    if (!firstField) return;

    if (firstField === "colorId") {
      formRef.current?.querySelector<HTMLElement>('input[name="colorId"]')?.focus();
    } else {
  ...
  return (
    ...
    >
      ...
      <div className="field">
        ...
        />
        {fieldErrors.colorId && <span className="field-error">{fieldErrors.colorId}</span>}
      </div>
